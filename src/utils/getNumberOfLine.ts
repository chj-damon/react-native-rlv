...
export default function getNumberOfLines(str: string, fontSize: number, width: number) {
  if (!str) {
    return 0;
  }
...
export function getCharWidth(
  char: string,
  { capitalWidth, lowerWidth, spaceWidth, numberWidth, chineseWidth }: ChartWidthType,
) {
  let width;
