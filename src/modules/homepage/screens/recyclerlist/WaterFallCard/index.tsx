...
export const queryRecyclerAnimals = async ({ pageParam = 0 }) => {
  ...
  const items = data.items.map((item: any, index: number) => {
    ...
    // 最多 2 行
    const numberOfLines = Math.min(getNumberOfLine(item.motto, titleSize, titleWidth), 2);
    ...
    const likedStr = item.liked < 1000 ? item.liked.toString() : Math.round(item.liked / 1000).toString() + 'k';
